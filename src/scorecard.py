...
def get_scorecard_data():
    ...
    if not closed_df.empty:
        ...
        stats["expectancy"] = (len(wins)/len(closed_df) * avg_win) + (len(losses)/len(closed_df) * avg_loss)

        # Market Breakdown
        m_stats = closed_df.groupby('Market').agg(
            Trades=('id', 'count'),
            Net_PnL=('pnl', 'sum'),
            Win_Rate=('pnl', lambda x: (x > 0).mean() * 100)
        ).sort_values(by='Net_PnL', ascending=False).reset_index()
        ...
        if 'confidence' in closed_df.columns:
            ...
            c_stats = closed_df.groupby('confidence').agg(
                Trades=('id', 'count'),
                Win_Rate=('pnl', lambda x: (x > 0).mean() * 100),
                Avg_PnL=('pnl', 'mean')
            ).reset_index()
...
def generate_scorecard():
    ...
    print(f"  > AI Waits:        {stats['ai_waits']} ({stats['ai_waits']/stats['total_sessions']*100:.1f}%)")
    print(f"  > Safety Rejects:  {stats['rejected']} ({stats['rejected']/stats['total_sessions']*100:.1f}%)")
    print(f"  > AI Errors:       {stats['ai_errors']}")
    print(f"Total Trades Taken:  {stats['total_trades']} (Conv: {stats['total_trades']/stats['total_sessions']*100:.1f}%)")

...
class TradeMonitorDB:
    ...
    def _handle_trade_update(self, data: dict):
        ...
        deal_status = payload.get(
            "dealStatus"
        )  # ACCEPTED, REJECTED etc (from CONFIRMS)
        ...
        logger.info(
            f"DEBUG: Trade Update. Deal: {deal_id}, Affected: {affected_deal_id}, Status: {trade_status}, DealStatus: {deal_status}, MonitoredID: {monitored_id}"
        )

        if not monitored_id:
            logger.info(
                f"Received trade update for unmonitored deal ID {deal_id} or no deal ID."
            )
            return
        ...
        if trade_status == "CLOSED" or trade_status == "DELETED":
            logger.info(
                f"STREAM: Trade {monitored_id} detected as {trade_status} via streaming update (OPU/CONFIRMS)."
            )
        ...
        # Let's rely on 'status' == 'CLOSED' which appears in both OPU (Position Closed) and CONFIRMS (Trade Closed).
            ...
            logger.info(
                f"STREAM: Trade {monitored_id} detected as CLOSED via CONFIRMS (AffectedDeal)."
            )
            self._active_monitors[monitored_id].set()
            ...
            logger.debug(
                f"STREAM: Unhandled trade status for {deal_id}: {trade_status} - {payload}"
            )
    ...
    def monitor_trade(
        ...
        logger.info(
            f"Starting Monitor & Manage for Deal {deal_id}. Risk: {entry_price}->{stop_loss}, ATR: {atr}, Trailing: {use_trailing_stop}"
        )
        ...
        # Calculate a minimum step for stop updates to avoid API spamming for tiny moves.
        # Prefer 10% of ATR, fallback to 5% of initial risk distance.
        if atr and atr > 0:
            min_step = 0.1 * atr
        elif risk_distance > 0:
            min_step = 0.05 * risk_distance
        else:
            min_step = 0.0  # Allow any move if we have no volatility context
                    ...
                    if timedelta(seconds=0) < time_to_close < timedelta(minutes=15):
                        ...
                        try:
                            ...
                            if pos:
                                ...
                                logger.warning(
                                    f"Could not fetch position {deal_id} for forced exit. It may be already closed."
                                )
                                break  # Assume closed if not found
                        except Exception as e:
                            logger.error(
                                f"Failed to force close trade {deal_id} near market close: {e}"
                            )
                ...
                if use_trailing_stop and risk_distance > 0:
                    try:
                        ...
                        if position:
                            ...
                            current_price = (
                                current_bid if direction == "BUY" else current_offer
                            )
                            profit_dist = (
            ...
            for attempt in range(10):
                ...
                try:
                    ...
                    if history_df is not None and not history_df.empty:
                        ...
                        logger.info(
                            f"History Fetch Attempt {attempt + 1}: Found PnL={final_pnl}, Exit={final_exit_price}"
                        )
                    ...
                    logger.warning(
                        f"History fetch attempt {attempt + 1} failed for {deal_id}: {e}"
                    )

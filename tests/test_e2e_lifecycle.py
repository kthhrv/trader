...
def test_trade_lifecycle_flow(lifecycle_db):
    ...
    monitor_db = TradeMonitorDB(mock_client, mock_stream, db_path=lifecycle_db, market_status=mock_market_status)
    ...
    mock_analyst.analyze_market.return_value = TradingSignal(
        ticker="TEST.EPIC", action=Action.BUY, entry=100, stop_loss=90, take_profit=120,
        confidence="high", reasoning="Lifecycle Test", size=1, atr=5.0, entry_type=EntryType.INSTANT,
    ...
    mock_client.place_spread_bet_order.return_value = {'dealId': 'REAL_DEAL_123', 'dealStatus': 'ACCEPTED'}
    mock_client.service.fetch_market_by_epic.return_value = {'snapshot': {'bid': 99, 'offer': 100}} # Spread ok
...
def test_trade_lifecycle_timeout(lifecycle_db):
    ...
    # 1. PENDING
    mock_analyst.analyze_market.return_value = TradingSignal(
        ticker="TIMEOUT.EPIC", action=Action.BUY, entry=100, stop_loss=90, take_profit=120,
        confidence="high", reasoning="Timeout Test", size=1, atr=5.0, entry_type=EntryType.INSTANT,
        use_trailing_stop=True
    )

...
def create_engine(epic, strategy_name, db_path, deal_id):
    ...
    mock_trade_monitor = TradeMonitorDB(client=mock_ig_client, stream_manager=mock_stream_manager, db_path=db_path, polling_interval=0.1)
    
    # Setup typical plan
    mock_gemini_analyst.analyze_market.return_value = TradingSignal(
        ticker=epic, action=Action.BUY, entry=100.0, stop_loss=90.0, 
        take_profit=120.0, confidence="high", reasoning="Test", size=1.0, atr=5.0,
    ...
    mock_ig_client.get_account_info.return_value = pd.DataFrame([
        {'accountId': 'MOCK_ACC', 'accountType': 'SPREADBET', 'preferred': True, 'available': 10000.0}
    ])
    mock_ig_client.service.account_id = 'MOCK_ACC'
    
    # Set unique deal ID
    mock_ig_client.place_spread_bet_order.return_value = {'dealId': deal_id, 'dealStatus': 'ACCEPTED'}
    ...
    engine = StrategyEngine(
        epic, strategy_name=strategy_name, ig_client=mock_ig_client, analyst=mock_gemini_analyst,
...
def test_concurrent_trades(temp_db_path, caplog):
    ...
    london_engine, london_ig, london_stream = create_engine(london_epic, "LONDON", temp_db_path, london_deal_id)
    ...
    london_thread = Thread(target=london_engine.execute_strategy, kwargs={'timeout_seconds': 5}, daemon=True)
    london_thread.start()
    time.sleep(0.2)
    ...
    ny_thread = Thread(target=ny_engine.execute_strategy, kwargs={'timeout_seconds': 5}, daemon=True)
    ny_thread.start()
    time.sleep(0.2)
    ...
    london_stream.simulate_trade_update({
        "dealId": london_deal_id,
        "status": "CLOSED"
    })
    ...
    assert not london_thread.is_alive(), "London thread should have finished after closure."
    ...
    ny_stream.simulate_trade_update({
        "dealId": ny_deal_id,
        "status": "CLOSED"
    })

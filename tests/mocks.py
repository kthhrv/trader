...
class MockIGClient:
    def __init__(self):
        ...
        self.fetch_historical_data_by_range = MagicMock(return_value=self.historical_data_df)
        
        # Default mock for place_spread_bet_order
        self.place_spread_bet_order = MagicMock(return_value={'dealId': 'MOCK_DEAL_ID', 'dealStatus': 'ACCEPTED'})
        
        # Default mock for get_account_info
        self.get_account_info = MagicMock(return_value=pd.DataFrame({
        ...
        self.fetch_open_position_by_deal_id = MagicMock(side_effect=self._mock_fetch_open_position)

        # Mock for transaction history for P&L
        self.fetch_transaction_history_by_deal_id = MagicMock(return_value=self._create_mock_transaction_history())
    ...
    def _create_mock_historical_data(self, num_points=50):
        ...
        df.index = pd.to_datetime([datetime.now() - timedelta(minutes=(num_points-1-i)*15) for i in range(num_points)])
        return df
    ...
    def simulate_position_close(self, pnl: float = 50.0):
        ...
        self.fetch_transaction_history_by_deal_id.return_value = self._create_mock_transaction_history(pnl=pnl)
...
class MockNewsFetcher:
    def __init__(self):
        self.fetch_news = MagicMock(return_value="Mock news context: Market looking stable.")

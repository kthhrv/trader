...
class TestTradeMonitorDBDeleted(unittest.TestCase):
    ...
    def test_monitor_trade_deleted_status(self, mock_update_db, mock_sleep):
        ...
        mock_history_df.iloc.__getitem__.return_value = {'profitAndLoss': '-10.0', 'closeLevel': 90.0}
        ...
        self.assertFalse(monitor_thread.is_alive(), "Monitor thread failed to exit on DELETED status")
        
        # Check DB update was triggered
        mock_update_db.assert_called_once()
        kwargs = mock_update_db.call_args[1]
        self.assertEqual(kwargs['outcome'], "CLOSED") # Our code sets final status to CLOSED even if trigger was DELETED

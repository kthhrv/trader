...
def e2e_mocks(temp_db_path):
    ...
    mock_news_fetcher = MagicMock() # NewsFetcher doesn't interact with DB directly, so MagicMock is fine
...
def test_e2e_trading_flow(e2e_mocks, caplog):
    ...
    mock_ig_client, mock_gemini_analyst, mock_stream_manager, mock_market_status, mock_news_fetcher, mock_trade_logger, mock_trade_monitor, db_path = e2e_mocks
    ...
    mock_historical_df = pd.DataFrame({
        ...
        'close': [entry_price - 8] * 24 + [entry_price + 3] # Make the last close different
    })
    mock_historical_df['ATR'] = [10.0] * 25
    mock_historical_df['RSI'] = [55.0] * 25
    mock_historical_df['EMA_20'] = [entry_price] * 25
    mock_historical_df.index = pd.to_datetime([datetime.now() - timedelta(minutes=(25-1-i)*15) for i in range(25)])
    ...
    trade_execution_thread = Thread(target=engine.execute_strategy, kwargs={'timeout_seconds': 3.0})

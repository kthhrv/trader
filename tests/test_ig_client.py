...
def test_authenticate_success(mock_ig_service):
    ...
    mock_instance.create_session.return_value = (
        None  # create_session doesn't return anything specific
    )
...
def test_authenticate_fail_account_not_found(mock_ig_service):
    ...
    with patch.object(config, "IG_ACC_ID", "NON_EXISTENT_ACC_ID"):
        ...
        with pytest.raises(
            Exception,
            match="Configured Account ID .* not found in DEMO TRADING accounts.",
        ):
            client.authenticate()
...
def test_fetch_historical_data_success(mock_ig_service):
    ...
    mock_instance.fetch_historical_prices_by_epic_and_num_points.return_value = {
        "prices": mock_df
    }
...
def test_place_spread_bet_order_success(mock_ig_service):
    ...
    confirmation = client.place_spread_bet_order(
        epic="CS.D.FTSE.TODAY.IP", direction="BUY", size=1, stop_level=7450
    )
...
def test_close_open_position_success(mock_ig_service):
    ...
    response = client.close_open_position(
        deal_id="DEAL123", direction="SELL", size=1, epic="CS.D.FTSE.TODAY.IP"
    )

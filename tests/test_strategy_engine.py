...
def test_poll_market_triggers_buy(mock_components):
    (
        ...
        mock_stream_manager,
        _,
    ) = mock_components  # Unpack all components
    ...
    # Patch _calculate_size at the class level
    with patch.object(
        StrategyEngine, "_calculate_size", return_value=1.0, create=True
    ) as mock_calculate_size:
        ...
        mock_stream_manager.connect_and_subscribe.side_effect = (
            mock_connect_and_subscribe
        )
    ...
    assert log_trade_args["is_dry_run"] is False
...
def test_place_market_order_dry_run(mock_components, caplog):
    ...
    with patch.object(
        ...
        with caplog.at_level(logging.INFO):
            ...
            mock_calculate_size.assert_called_once()
            assert (
                engine.position_open is True
            )  # Position open becomes True to stop polling
        ...
        assert log_trade_args["is_dry_run"] is True

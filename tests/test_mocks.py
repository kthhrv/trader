...
def test_mock_ig_client_fetch_historical_data(mock_ig_client_instance):
    ...
    assert not df.empty
    mock_ig_client_instance.fetch_historical_data.assert_called_once_with(
...
def test_mock_ig_client_place_spread_bet_order(mock_ig_client_instance):
    ...
    response = mock_ig_client_instance.place_spread_bet_order(
        epic, direction, size, stop_level, None, limit_level
    )

    assert response == {"dealId": "MOCK_DEAL_ID", "dealStatus": "ACCEPTED"}
    mock_ig_client_instance.place_spread_bet_order.assert_called_once_with(
        epic, direction, size, stop_level, None, limit_level
    )
...
def test_mock_ig_client_fetch_transaction_history_by_deal_id(mock_ig_client_instance):
    ...
    pnl_value = 123.45
    mock_ig_client_instance.simulate_position_close(
        pnl=pnl_value
    )  # Ensure history mock is updated
...
def test_mock_gemini_analyst_analyze_market(mock_gemini_analyst_instance):
    ...
    signal = mock_gemini_analyst_instance.analyze_market(
        market_context, strategy_name=strategy_name
    )
    ...
    mock_gemini_analyst_instance.analyze_market.assert_called_once_with(
        market_context, strategy_name=strategy_name
    )
...
def test_mock_gemini_analyst_generate_post_mortem(mock_gemini_analyst_instance):
    ...
    mock_gemini_analyst_instance.generate_post_mortem.assert_called_once_with(
        trade_data
    )
...
def test_mock_stream_manager_connect_and_subscribe(mock_stream_manager_instance):
    ...
    mock_stream_manager_instance.connect_and_subscribe.assert_called_once_with(
        epic, mock_callback
    )
...
def test_mock_trade_logger_db_log_trade(mock_trade_logger_instance):
    ...
    mock_trade_logger_instance.log_trade(
        epic, plan, outcome, spread_at_entry, is_dry_run, deal_id
    )
    mock_trade_logger_instance.log_trade.assert_called_once_with(
        epic, plan, outcome, spread_at_entry, is_dry_run, deal_id
    )

...
class TestDatabase(unittest.TestCase):
    ...
    def test_fetch_recent_trades(self):
        ...
        try:
            ...
            # Insert dummy trades
            cursor.execute(
                "INSERT INTO trade_log (timestamp, epic, action, entry, deal_id) VALUES ('2023-01-01T10:00:00', 'A', 'BUY', 100, '1')"
            )
            cursor.execute(
                "INSERT INTO trade_log (timestamp, epic, action, entry, deal_id) VALUES ('2023-01-01T11:00:00', 'B', 'SELL', 200, '2')"
            )
            cursor.execute(
                "INSERT INTO trade_log (timestamp, epic, action, entry, deal_id) VALUES ('2023-01-01T12:00:00', 'C', 'BUY', 300, '3')"
            )
            conn.commit()
            conn.close()
    ...
    def test_fetch_trade_data_and_save_post_mortem(self):
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO trade_log (deal_id, epic) VALUES (?, ?)",
                (deal_id, "TEST.EPIC"),
            )
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT post_mortem FROM trade_log WHERE deal_id=?", (deal_id,)
            )
            row = cursor.fetchone()

...
class TestStreamManager(unittest.TestCase):
    ...
    def test_connect_success(self, mock_thread_cls, mock_popen):
        ...
        with patch.object(
            self.stream_manager.is_connected, "wait", return_value=True
        ) as mock_wait:
            ...
            mock_thread_cls.assert_called_once_with(
                target=self.stream_manager._read_stdout, args=(mock_stdout_pipe,)
            )
    ...
    def test_connect_timeout(self, mock_thread_cls, mock_popen):
        ...
        with patch.object(
            self.stream_manager.is_connected, "wait", return_value=False
        ) as mock_wait:
